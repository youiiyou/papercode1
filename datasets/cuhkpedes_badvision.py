...
class CUHKPEDES(BaseDataset):
    ...
    def _split_anno(self, anno_path: str):
        ...
        for anno in annos:
            ...
            folder = op.dirname(file_path)

            if folder == 'train_query':
                train_annos.append(anno)
            elif folder == 'test_query':
    ...
    def _process_anno(self, annos: List[dict], training=False):
        ...
        if training:
            ...
            for anno in annos:
                raw_pid = int(anno['id'])
                pid_container.add(raw_pid)
            
            pid2label = {pid: idx for idx, pid in enumerate(sorted(pid_container))}

            for anno in annos:
                raw_pid = int(anno['id']) # make pid begin from 0
    ...
    def _check_before_run(self):
        ...
        key_subfolders = ['cam_a', 'cam_b', 'CUHK01', 'CUHK03', 'Market', 'test_query', 'train_query']
        for subfolder in key_subfolders:
            subfolder_path = op.join(self.img_dir, subfolder)
            if not op.exists(subfolder_path):
                raise RuntimeError("'{}' is not available (key subfolder under imgs/)".format(subfolder_path))
